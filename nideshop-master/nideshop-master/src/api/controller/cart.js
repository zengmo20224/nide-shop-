...
module.exports = class extends Base {
  ...
  async updateAction() {
    ...
    const cartInfo = await this.model('cart').where({id: id, user_id: this.getLoginUserId()}).find();
    if (think.isEmpty(cartInfo)) {
      return this.fail(400, '购物车商品不存在');
    }
    // 只是更新number
    if (cartInfo.product_id === productId) {
      await this.model('cart').where({id: id, user_id: this.getLoginUserId()}).update({
        number: number
      });
    ...
    const newCartInfo = await this.model('cart').where({goods_id: goodsId, product_id: productId, user_id: this.getLoginUserId()}).find();
    if (think.isEmpty(newCartInfo)) {
      ...
      await this.model('cart').where({id: id, user_id: this.getLoginUserId()}).update(cartData);
    } else {
      ...
      await this.model('cart').where({id: newCartInfo.id, user_id: this.getLoginUserId()}).delete();
  ...
  async checkedAction() {
    ...
    await this.model('cart').where({product_id: {'in': productId}, user_id: this.getLoginUserId()}).update({checked: parseInt(isChecked)});
  ...
  async deleteAction() {
    ...
    await this.model('cart').where({product_id: {'in': productId}, user_id: this.getLoginUserId()}).delete();
