...
module.exports = class extends think.Model {
  ...
  async updatePayStatus(orderId, payStatus = 0, extra = {}) {
    const data = Object.assign({}, extra, {
      pay_status: parseInt(payStatus)
    });
    if (parseInt(payStatus) === 2) {
      data.order_status = 201;
      data.pay_time = parseInt(Date.now() / 1000);
    }
    return this.where({id: orderId}).limit(1).update(data);
  }
