  ...
  getGoodsInfo: function () {
    ...
    util.request(api.GoodsDetail, { id: that.data.id }).then(function (res) {
      if (res.errno === 0) {
        ...
        let gallery = Array.isArray(detail.gallery) ? detail.gallery : [];
        let coverImage = info.primary_pic_url || info.list_pic_url || '';

        if (!coverImage && gallery.length > 0 && gallery[0].img_url) {
          coverImage = gallery[0].img_url;
        }
        ...
        WxParse.wxParse('goodsDetail', 'html', info.goods_desc || '', that);
